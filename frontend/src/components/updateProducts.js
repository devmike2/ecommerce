...
const UpdateProduct = ({
  ...
  const handleSubmit = async(e) =>{
      ...
      const fetchData = await fetch(routeApi.UpdateProduct.url, {
        method: routeApi.UpdateProduct.method,
        credentials: "include",
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
         reqBody: data,
         reqId: updatedata._id
        })
      })
      ...
      <div className="fixed bg-slate-200 bg-opacity-40 h-full inset-0 w-full z-10 flex justify-center items-center gap-2">
         <div className="bg-white p-4 w-full h-full max-w-2xl max-h-[calc(100vh-150px)] overflow-hidden">
            <div className="flex justify-between pb-3">
               <h2 className="text-2xl font-bold">Update Products</h2>
            ...
            <form className="grid gap-3 mb-3 p-4 overflow-y-scroll h-full" onSubmit={handleSubmit}>
               ...
               <label htmlFor="brandName" className="text-lg nt-3">brandName : </label>
               <input 
               ...
               <textarea className="h-32 border Px-2 py-1 bg-slate-100 resize-none" name="description" onChange={handleChange} id="description" value={data.description} placeholder="Enter a descrption"></textarea>


               {/* ============================== submit button =============================== */}
               <button className="px-3 py-4 bg-green-600 text-white hover:bg-green-700 mb-10 mt-3" >Update Product</button>
